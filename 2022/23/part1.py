...
width = max(elf_positions, key=lambda x:x[1])[1] - min(elf_positions, key=lambda x:x[1])[1] + 1
height = max(elf_positions, key=lambda x:x[0])[0] - min(elf_positions, key=lambda x:x[0])[0] + 1

print(width * height - len(elf_positions))