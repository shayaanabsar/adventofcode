...
def get_neighbours(curr, time, blizzards):
	...
	for neighbour in poss_neighbours:
		i, j = neighbour
		if (neighbour not in blizzard_set and ((0 <= i < len(data)) and (0 <= j < len(data[0])))) or (neighbour == end) or (neighbour == start):
...
journey1 = bfs(start, end, 0)
journey2 = bfs(end, start, journey1)
journey3 = bfs(start, end, journey2)
print(journey3)