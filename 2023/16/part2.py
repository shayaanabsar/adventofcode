...
def test(start_i, start_j, dir):
	queue = deque([(start_i, start_j, dir)])
	seen_positions = set()
...
for i in range(width):
	highest = max(highest, test(0, i, 180))
	highest = max(highest, test(height-1, i, 90))

for j in range(height):
	highest = max(highest, test(j, 0, 90))
	highest = max(highest, test(j, width-1, 270))
print(highest)
